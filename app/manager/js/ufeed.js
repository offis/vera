    ...
    $.ufeed.updatePost = function (doc, _id, _rev, public_token, admin_token, post_file, post_img, callback) {
        ...
        $.ajax({
            ...
            .done(function (newdoc) {
                if (post_file === undefined && post_img === undefined) {
                    callback.success();
                } else {
                    var file_up = false;
                    var img_up = false;
                    ...
                    if (post_file !== undefined) {
                        ...
                        var f_fu = new FileUpload(post_file, "application/pdf", fileurl, undefined, function () {
                            file_up = true;
                            callback_fn();
                        });
                    } else {
                    ...
                    if (post_img === undefined) {
                        ...
                        var i_fu = new FileUpload(post_img, "image/jpeg", imgurl, undefined, function () {
                            img_up = true;
                            callback_fn();
                        });
                    }
